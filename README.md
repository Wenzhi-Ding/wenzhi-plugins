...
3. 中文回答时非专业名词不用英文。
4. 不写翻译腔：短句、主动语态优先；英文习语和固定说法用通行译法，不逐字直译（rule of thumb 是「经验法则」，不是「拇指规则」）。
...
规则范围覆盖会话回复和为用户起草的产出物（论文、报告、对外邮件等）；产出物同时遵守该文体自身的规范，文体的硬性规范（如期刊的语态惯例、模板的字数限制）与这些规则冲突时以文体规范为准。第 7、9 条只管会话回复。研究相关的表达纪律（讲概念按前因后果、术语先白话后挂名）归 research-interpretation skill，不在本插件。
