...
规则范围覆盖中英文的会话回复和为用户起草的产出物（论文、报告、对外邮件等）；产出物同时遵守该文体自身的规范，文体的硬性规范（如期刊的语态惯例、模板的字数限制）与这些规则冲突时以文体规范为准。规则只给普适原则，不带场景化条款和错/对示例——需要完整示例版（约 2000 字）见 git 标签 `humanize-v0.6.0`；唯一例外是第 7 条研究观察与结果记录的口径要求（始终读者可懂、内部代号首现展开、结论与证据来源对应可查）。研究相关的其余表达纪律（讲概念按前因后果、术语先白话后挂名）归 research-interpretation skill。
